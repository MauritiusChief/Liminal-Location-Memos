...
剔除视野遮挡物体发生在扁平结构组装之后，分级与分层之前

此处定义因高度而必定被看到的地物（同时用在此处以及后续 Polar View Filter）

视野遮挡机制：
1. 所有 level 1 (30~100m), level 2 (100~300m), level 3a (300m~500m) 建筑计算可见角度区间
2. 对 level 2, level 3a, level 3b (500m~1km) 每个地物应用所有内侧可见区间
3. 对 level 2 和 3a 的每个地物，判断是否有点能透过可见角度区间被极坐标原点看到，有一点可见便可见
4. 对 level 3b 的每个地物，判断是否完全处在可见区间中，完全可见才算可见
5. 把因高度而必定显著的地物额外加上

## Polar View Filter

这是加在 Polar View Object 上的的过滤器，来模拟细节看不清的情况
