
## 近期计划

* 获取数据时不要排除 relation，不然有些复合建筑与超长道路会被底下的小结构重复算
* 更新 normalization 功能，尽量把 relation 消除掉。这样还可以避免线状地物爆炸
  * 但同时得想办法把tag也保留
* 把 level 和 building:level 统一
* 粗看保留更多近处物体
* 粗看细看也许可以应用随机筛选
* 添加通过其他 api 的海拔信息计算 ele 标签的建筑的高度
* 添加记忆功能/工具，可以把觉得重要的东西记下来。
* 主对话再调整一下，如果不是特别要求查看周围，就再压缩环境描写、扩充“你”的动作与行为描写，让被压缩的环境描写显得像是匆匆走过没有细看
* 考虑如何实现汽车这样 10 分钟就能走 10 公里的速度如何实现
  * 大概率得想办法长距离组装线状特征，然后沿着线状特征的坐标行进
  * 或者调整调用工具的模式，进行连续调用工具快速移动（然后每次调用工具根据速度略微描写环境，速度越快描写越少）

## 长远计划

* 改用 stream，避免超长时间等待
* large description 与 small description 看怎么样也改成 stream，即能与前端互动又能作为总体 chat turn 的一部分